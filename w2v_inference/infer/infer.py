...
output units",
    ...
    parser.add_argument("--beam-threshold", type=float, default=250.0)
    parser.add_argument("--beam-size-token", type=float, default=1000)
...
def main(args, task=None, model_state=None):
    check_args(args)
    fn = args.results_path.split('/')
    folname = '/'.join(fn[:-1])
    fname = '_'.join(fn[-1].split('_')[2:])
    f = open(os.path.join(folname,fname+'_logs.txt'),'a')
    ...
    def build_generator(args):
        ...
        if w2l_decoder == "viterbi":
            from inference.support import W2lViterbiDecoder

            return W2lViterbiDecoder(args, task.target_dictionary)
        elif w2l_decoder == "kenlm":
            from inference.support import W2lKenLMDecoder
    ...
    if args.dump_emissions:
        ...
        f.write("| Generate {} with beam={}\n\n\n".format(args.gen_subset, args.beam))
    return task, wer
